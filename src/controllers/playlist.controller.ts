...
const getPlaylistById = asyncHandler(async (req, res) => {
    ...
    // Return the Playlist
    return res.status(200).json(new ApiResponse(200, fetchedplaylist, "playlist fetched successfully"));
});
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    const updatedplaylist = await Playlist.findByIdAndUpdate(req.playlist?._id, req.body, { new: true });
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if (req.playlist?.videos.includes(req.video?._id as any))
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    if (!req.playlist?.videos.includes(req.video?._id as any))
        throw new ApiError(400, "Video you want to remove does not exist in the playlist");
